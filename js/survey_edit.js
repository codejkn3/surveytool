...
function validateForm4() { //This works. Use for comparison
    var xhr = new XMLHttpRequest();
    var new_survey_name = $('#survey_name').val();
 
    if (new_survey_name.length == 0)
    {
        alert('Please enter the survey title');
        return false;
    }

    xhr.open('GET', 'validate.php?input_survey_name='+new_survey_name,false); //async = false is deprecated
    xhr.send();
    if (xhr.status === 200) {
        //alert('Status is ' + xhr.responseText);
        if (xhr.responseText == 'true') { /* we found this name in the DB */
            alert(new_survey_name + " is already in use. Please choose another name.");
            return false;
        }
    }
